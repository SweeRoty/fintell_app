...
if __name__ == '__main__':
	...
	records = records.join(devices, on=['imei'], how='left_outer').where(F.col('flag').isNotNull())
	apps = records.drop('imei').repartition(1000, 'app_package').rdd.map(lambda row: (row['app_package'].encode('utf-8'), 1)).reduceByKey(lambda x, y: x+y)
